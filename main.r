...
set.seed(1237)
control <- trainControl(method = "cv", number = 5)
tgrid <- expand.grid( mtry = c(3:10), spliturle = "variance", min.node.size = c(10,15,20))
rf_mod <- train(x=train[,-c("Item_Identifier")], y=train$Item_Outlet_Sales, method="ranger", trControl=control, tuneGrid=tgrid, num.trees=400, importance="permutation")
...
dtrain <- xgb.DMatrix(data = as.matrix(train[,-c("Item_Identifier")]), label = train$Item_Outlet_Sales)
